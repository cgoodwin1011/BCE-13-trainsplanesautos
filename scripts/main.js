...
function makeTrain(name, destination, firstTime, intervalBetween) {
  var guid = moment().unix();
  returnValue = {guid: guid, name: name, destination: destination, firstTime: firstTime, interval: intervalBetween};
  return returnValue;
}
...
function displayTrain(aTrain) {
  ...
  var nextTrain = nextTrainTime(aTrain.firstTime, aTrain.interval);
...
function nextTrainTime(startTime, interval) {
  var now = moment();
  var unixStartTime = moment(startTime, "HH:mm")
  var timeSoFarToday = now.diff(unixStartTime)
  unixInterval = interval*60*1000;
  var remaninder = (timeSoFarToday % unixInterval);
  var timeTillNext = unixInterval - remaninder;
  var priorTrain = now - (timeSoFarToday % unixInterval);
  var nextTrain = moment(now+timeTillNext).format("HH:mm");
  return nextTrain;
}
